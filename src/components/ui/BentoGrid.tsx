...
export const BentoGridItem = ({
  ...
  const LottieComponent = dynamic(() => import('./LottieAnimation'), { ssr: false })
    ...
    <div className={`${id === 6 && "flex justify-center"} h-full`}>
      ...
      <div
       ...
       >
        ...
        {/* Tech stack list div */}
        {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute top-0 -right-3 lg:-right-2 px-4">
            {/* tech stack lists */}
            <div className="flex flex-col gap-3 pt-2 lg:gap-5 ">
          {leftLists.map((item)=>(
            <span key={item} className="py-1 px-3 text-sm lg:text-base opacity-50 lg:opacity-50 rounded-lg text-center bg-[#10132e] ">{item}</span>
          ))}
          {/* <span className="py-2 px-3 rounded-lg text-center bg-[#10132e] "> </span> */}
              ...
              {rightLists.map((item, i) => (
                <span
                  key={i}
                  className=" py-1 px-3 text-sm lg:text-base opacity-50 
...
<div className="w-full h-full absolute">
    ...
    <div className={cn(titleClassName," group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40  flex flex-col p-5 lg:p-10")}>
      <div className="font-sans font-extralight text-[#c1c2d3] text-sm lg:text-base md:text-xs z-10 ">
        {description}
      </div>
    ...
    { id == 3 && 
      <div className=" flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
        <div className="flex flex-col gap-3 lg:gap-8 ">
          {.map((item)=>(
            <span key={item} className="py-2 lg:py-4 lg:px-4 px-3 text-xs lg:text-base opacity-50 lg:opacity-50 rounded-lg text-center bg-[#10132e]  ">{item}</span>
          ))}
